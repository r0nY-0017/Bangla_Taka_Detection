...
try:
    model_dict = pickle.load(open('./model_svm.p', 'rb'))
...
class BanknoteDetectorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Banknote Detector (ORB + SVM)")
    ...
    def detect_banknote(self, descriptors, keypoints):
        if descriptors is not None and len(keypoints) >= MIN_KEYPOINTS:
            descriptor_features = descriptors.flatten()
            FIXED_FEATURE_SIZE = 27232
            if len(descriptor_features) < FIXED_FEATURE_SIZE:
                descriptor_features = np.pad(descriptor_features, (0, FIXED_FEATURE_SIZE - len(descriptor_features)), mode='constant')
            else:
                descriptor_features = descriptor_features[:FIXED_FEATURE_SIZE]
